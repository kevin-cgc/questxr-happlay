...
#include "frag.spv"
	...
	void RenderView(const XrCompositionLayerProjectionView &layer_view,
		...
		glm::mat4 view = math::InvertRigidBody(
			glm::translate(glm::identity<glm::mat4>(), math::XrVector3FToGlm(layer_view.pose.position)) * glm::mat4_cast(math::XrQuaternionFToGlm(layer_view.pose.orientation)));
		std::vector<glm::mat4> transforms{};
		for (const math::Transform &cube : cube_transforms) {
			glm::mat4 model = glm::scale(glm::translate(glm::identity<glm::mat4>(), cube.position) * glm::mat4_cast(cube.orientation), cube.scale);
